...
# Read in data
d <- read.csv("main_data_v2.csv") # data for main analysis
...
d$y[d$wave == 3][d$y[d$wave == 3]==5] <- NA # (in wave 4, 5 = "prefer not to respond")
...
hist(d$y[d$wave == 3])
d$y[d$wave == 4] <- abs(d$y[d$wave == 4] - 5) # rescoring so that low responses are less religious
hist(d$y[d$wave == 4])
...
# when gender[wave==1] = NA, assume that gender is the same in wave 1 as in wave 2
...
d$gender[d$wave==2] <- ifelse(is.na(d$gender[d$wave==2]), d$gender[d$wave==1], d$gender[d$wave==2])
...
# when gender[wave==4] = NA, assume that gender is the same in wave 4 as in wave 1
d$gender[d$wave==4] <- ifelse(is.na(d$gender[d$wave==4]), d$gender[d$wave==1], d$gender[d$wave==4])
...
### Model 0: Unconditional change model 
...
## Fit Model 0 
m0 <- brm(y ~ 1 + mo(wave) + (1 + mo(wave) | id), # unconditional model (listwise deletion)
                  data = d_full,
                  family=cumulative("logit"),
                  prior = priors_m0,
...
d_complete <- d_full[complete.cases(d_full$y),] %>% group_by(id) %>% filter(n()>3) %>% as.data.frame() # include only participants who completed all four waves
...
d_full_complete <- d_full[complete.cases(d_full),] %>% group_by(id) %>% filter(n()>3) %>% ungroup() %>% as.data.frame() # include only participants who completed all four waves
...
pc_m_health <- brm(y ~ 1 + mo(wave) + mo(wave)*mo(health) + (1 + mo(wave) + mo(wave)*mo(health) | id), 
                       data = d_full,
                       family = cumulative("logit"), 
                       prior = priors_health,
                       chains = 2, iter = 1000,
...
yrep_pc_health <- posterior_predict(pc_m_health)

priorplot_health <- ppc_bars_grouped(y = as.numeric(pc_m_health$data[["y"]]),
                  yrep = yrep_pc_health,
...
# sample from the priors
pc_m_hinc <- brm(y ~ 1 + mo(wave) + mo(wave)*mo(hinc) + (1 + mo(wave) + mo(wave)*mo(hinc) | id), 
                       data = d_full,
                       family = cumulative("logit"), 
                       prior = priors_hinc,
                       chains = 2, iter = 1000,
...
yrep_pc_hinc <- posterior_predict(pc_m_hinc)

priorplot_hinc <- ppc_bars_grouped(y = as.numeric(pc_m_hinc$data[["y"]]),
                  yrep = yrep_pc_hinc,
...
# fit model
m_health <- brm(y ~ 1 + mo(wave) + mo(wave)*mo(health) + (1 + mo(wave) + mo(wave)*mo(health) | id), 
                        data = d_full,
                        family = cumulative("logit"), 
                        prior = priors_health,
                        seed = 2021,
...
yrep_health <- posterior_predict(pc_m_health)

postplot_health <- ppc_bars_grouped(y = as.numeric(pc_m_health$data[["y"]]),
                  yrep = yrep_health,
                  group = pc_m_health$data[["wave"]],
                  freq = FALSE)
...
# fit model
m_hinc <- brm(y ~ 1 + mo(wave) + mo(wave)*mo(hinc) + (1 + mo(wave) + mo(wave)*mo(hinc) | id), 
                        data = d_full,
                        family = cumulative("logit"), 
                        prior = priors_hinc,
                        seed = 2021,
...
yrep_hinc <- posterior_predict(m_hinc)

postplot_hinc <- ppc_bars_grouped(y = as.numeric(m_hinc$data[["y"]]),
                  yrep = yrep_hinc,
...
# actual poststratification takes places in script for supplementary plots
...
# as suggested by Burkner here: https://discourse.mc-stan.org/t/creating-a-brmsfit-object-with-a-modified-brms-generated-stan-model/6840
...
df_exp$y[df_exp$wave == 4][df_exp$y[df_exp$wave == 4]==5] <- NA # (in wave 3, 5 = "prefer not to respond")
df_exp$y[df_exp$wave == 4] <- abs(df_exp$y[df_exp$wave == 4] - 5) # rescoring so that low responses are less religious
...
save.image("pandemic_panel_fits_v2.RData")

### Model diagnostic checks

rstan::check_hmc_diagnostics(m0$fit)
rstan::check_hmc_diagnostics(m1$fit)
rstan::check_hmc_diagnostics(m0_complete$fit)
rstan::check_hmc_diagnostics(m1_complete$fit)
rstan::check_hmc_diagnostics(m_mrp$fit)
rstan::check_hmc_diagnostics(m1_imp$fit)
rstan::check_hmc_diagnostics(m_hinc$fit)
rstan::check_hmc_diagnostics(m_health$fit)
rstan::check_hmc_diagnostics(m_self$fit)
rstan::check_hmc_diagnostics(m_household$fit)
rstan::check_hmc_diagnostics(m_family$fit)
rstan::check_hmc_diagnostics(m_relations$fit)
