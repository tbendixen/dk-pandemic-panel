...
JointAI::plot_all(d_full[d_full$wave==4,]) # wave 4
...
digmat.perc <- matrix(c(0,0,0,0,0, rep(c(0,1, 1, 1, 1),(20))), 
                      nrow = 21, ncol=5, byrow=TRUE)

rownames(demo.perc) <- c("N", "mAge", "sdAge", "Gender (women)", 
                         "Primary education", "High school/Vocational training", "Short cycle higher education", 
                         "(Vocational) bachelors education", "Masters education", "Doctoral degree",
                         "< 100.000", "100.000 -- 199.999", "200.000 -- 299.999", "300.000 -- 399.999", 
                         "400.000 -- 499.999", "500.000 -- 599.999", "600.000 -- 699.999", "700.000 -- 799.999", 
                         "800.000 -- 899.999", "900.000 -- 999.999", "> 1.000.000")

xtable(demo.perc, digits = digmat.perc, align = c("l", "c", "c", "c", "c"),
...
### FIGURE 2: Posterior predictive draws
...
m1_fitted <- m1$data %>%
  modelr::data_grid(wave = seq_range(1:4, by = 1), age.c = 0, gender = c(2,1), edu = 3, health = 3, hinc = 6) %>%
  add_epred_draws(m1, ndraws = 300, newdata = ., re_formula = NA, resp = "y") %>%
...
m1_fitted %>%
  ...
  ggtitle("Posterior predictions for men (left) and women (right)", subtitle = "With covariates at their reference values") 

dev.off() # end print to pdf

### FIGURE 3: Posterior predictions for 15 individuals

id_plot_df <- d_full # copy full dataset

# only complete-case individuals so that the model can predict for covariates
id_plot_df_complete <- d_full[complete.cases(id_plot_df[4:9]),] %>% group_by(id) %>% filter(n()>3) %>% as.data.frame()

set.seed(1111) # make random sampling reproducible -- change seed to sample other IDs
...
cairo_pdf("Figure3.pdf", width = 6, height = 4.5) # start print to pdf
...
id_plot_ribbon_m1 <- id_plot_df_select %>%
  ...
  add_predicted_draws(newdata = ., object=m1, re_formula = NULL) %>%
  ggplot() +
  stat_lineribbon(aes(x = wave, y = as.numeric(.prediction), group = paste(id)), .width = c(.66, .97), alpha = .75, color = "#08519C",
  ...
  scale_fill_brewer() +
  scale_x_continuous(name="Wave", breaks=c(1,2,3,4)) +
  scale_y_continuous(name="''How important is religion in your life?''", breaks=c(1,2,3,4), limits = c(.8,4.2)) + 
  theme_minimal() +
  ...
  ggtitle("Posterior predictions for individual participants", subtitle = "15 pseudo-randomly sampled IDs with covariates at their observed values")

id_plot_ribbon_m1 # throws an error ("Removed X rows containing missing values (geom_point).") which can be ignored (it's just removing NAs in the outcome)
...
fixef_health <- as.data.frame(round(fixef(m_health, pars = coefadj), 2))
fixef_hinc <- as.data.frame(round(fixef(m_hinc, pars = coefadj), 2))
fixef_mrp <- as.data.frame(round(fixef(m_mrp, pars = coefadj), 2))
...
coef_list <- list(m0 = paste0(fixef_m0[,1], " [",fixef_m0[,3], ", ", fixef_m0[,4],"]"),
                       ...
                       m_hinc = paste0(fixef_hinc[,1], " [",fixef_hinc[,3], ", ", fixef_hinc[,4],"]"),
                 
                       m_health = paste0(fixef_health[,1], " [",fixef_health[,3], ", ", fixef_health[,4],"]"),
                  
                       m_mrp = paste0(fixef_mrp[,1], " [",fixef_mrp[,3], ", ", fixef_mrp[,4],"]"),
...
rownames(coef_tab) <- c("1st threshold", "2nd threshold", "3rd threshold", "Wave", "Wave x H. income", "Wave x Health")
